...
# function that removes words supplied with word_list_filter variable/argument
...
# function that converts sentences into words
...
You can get the script from here: [`topic_modeling_script02.py`](./files/tm/topic_modeling_script02.py). Make sure to change `dispatchSubfolder` if necessary. Keep in mind that running this script with a high number of topics (`maxTopic`) is likely to take quite a while.
...
You can get the script from here: [`topic_modeling_script03.py`](./files/tm/topic_modeling_script03.py). Make sure to change `dispatchSubfolder` if necessary. Keep in mind that running this script with a high number of topics (`maxTopic`) is likely to take quite a while.
...
The next script will run a series of analyses on the topic modeling data that we generated in the previous steps. These will include:

- graphing the distribution of topics over time;
- extract texts that are most representative of each topic;
